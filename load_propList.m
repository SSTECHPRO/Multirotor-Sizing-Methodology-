...
function output = load_propList()
    ...
    %[~, ~, everything] = xlsread('PROP-DATA-FILE_202201.xlsx'); % read from the data file
    %fileList= fopen('PERFILES_WEB/PER2_TITLEDAT.DAT', 'rt');

    fileList = [{} {}];
    for ii = 2:size(everything,1) % skip first 5 lines and save propeller names and respective filenames
        fileList(ii-1,:) = [everything(ii,2) everything(ii,1)];
    end

    clear everything; % clear unused data

    [~, ~, everything] = xlsread('PROP-DATA-FILE_202201.xlsx', 2); % read from the data file
